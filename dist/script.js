...
window.addEventListener('DOMContentLoaded', function () {
  ...
  new _modules_playVideo__WEBPACK_IMPORTED_MODULE_2__["default"]('.module .play', '.overlay').init();
...
function () {
  ...
  _createClass(Download, [{
    ...
    value: function downloadItem(path) {
      var link = document.createElement('a'); //искуственная ссылка
    ...
    value: function init() {
      ...
      this.btns.forEach(function (item) {
        ...
        item.addEventListener('click', function (e) {
          ...
          _this.downloadItem(_this.path); //запускаем ф-ю скачивания по определенному пути(this.path)
  ...
  _createClass(Forms, [{
    ...
    value: function initMask() {
      ...
      var setCursorPosition = function setCursorPosition(pos, elem) {
        //в pos будет кол-во символов //elem -this - объект с которым мы будем работать
      ...
      this.forms.forEach(function (form) {
        form.addEventListener('submit', function (e) {
          ...
          _this.postData('assets/question.php', formData).then(function (res) {
            console.log(res);
            statusMessage.textContent = _this.message.success; //плашка об успешности
          }).catch(function () {
            statusMessage.textContent = _this.message.failure; //плашка о неудаче   
          }).finally(function () {
            _this.clearInputs();
  ...
  _createClass(VideoPlayer, [{
      ...
      this.btns.forEach(function (btn, i) {
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            //если у селектора нет даты атрибута, то следует ф-я ниже //!btn.closest('.module__video-item') -проверка на сущ-е дабы избежать ошибки с плеером на главной странице
            _this.activeBtn = btn; //Решение проблемы с возпроизведением видео на фоне 

            if (document.querySelector('iframe#frame')) {
              //если уже мы вызывали плеер,т.е. возник селектор iframe#frame 
              _this.overlay.style.display = 'flex'; // то показываем модальное окно

              if (_this.path !== btn.getAttribute('data-url')) {
                _this.path = btn.getAttribute('data-url');

                _this.player.loadVideoById({
                  //loadVideoById - API-метод ютуба, позволяющий указать id video
                  videoId: _this.path
                });
              }
            } else {
              // но если видео нет
              _this.path = btn.getAttribute('data-url'); //обращение к пути объекта

              _this.createPlayer(_this.path); // то создаем его
    ...
    value: function bindCloseBtn() {
      ...
      this.close.addEventListener('click', function () {
        ...
        _this2.player.stopVideo(); //при закрытии останавливаем видео 
    ...
    value: function createPlayer(url) {
      this.player = new YT.Player('frame', {
        //YT-будет подтягиваться с серверов ютуба. //frame - уникальный id блока на нашей странице который будем замещать плеером
        height: '100%',
        //высота плеера
        width: '100%',
        //ширина плеера
        videoId: "".concat(url),
        //сюда будем подгружать уникальный ID видео который на ютубе
        events: {
    ...
    value: function onPlayerStateChange(state) {
      //срабатывает каждый раз когда изменяется состояние плеера //Ф-ю взяли из YoutubeAPI
      try {
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling,
            //получаем след соседний блок
        ...
        if (state.data === 0) {
          //описание исп-я data указно в документации YoutubePlayer //0-значит видео завершено
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            //устранения ошибки с повторным просмотром видео
    ...
    value: function init() {
      ...
      if (this.btns.length > 0) {
        var tag = document.createElement('script'); //создает тег скрипт

        tag.src = "https://www.youtube.com/iframe_api"; //внутри помещает ссылку

        var firstScriptTag = document.getElementsByTagName('script')[0]; //yghzvj на странице находим первый скрипт, кот-й у нас есть
...
function () {
  ...
  _createClass(ShowInfo, [{
    key: "init",
    value: function init() {
      this.btnsPlus.forEach(function (btn) {
        btn.addEventListener('click', function () {
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function showSlides(n) {
      ...
      try {
        //try/catch используем так как в других слайдах нет hanson
    ...
    key: "toggleButtons",
    value: function toggleButtons(buttons, n) {
      var _this3 = this;
    ...
    value: function bindTriggers() {
      ...
      this.btns.forEach(function (btn) {
        btn.addEventListener('click', function () {
          _this4.slides.forEach(function (slide) {
        ...
        btn.parentNode.previousElementSibling.addEventListener('click', function (e) {
          ...
          _this4.showSlides(_this4.slideIndex); // при клике будет первый слайд
    ...
    value: function render() {
      if (this.container) {
        //условие для избежания ошибки с container //если есть такой container 
        try {
          //try/catch т.к. в других слайдах нет hanson
...
function (_Slider) {
  ...
  _createClass(MiniSlider, [{
    ...
    value: function decorizeSlides() {
      ...
      this.slides.forEach(function (slide) {
        slide.classList.remove(_this2.activeClass); //Скрытие стрелки и тайтла на неактивных карточках
    ...
    value: function bindTriggers() {
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this4.slides.length - 1; i > 0; i--) {
          //запуск цикла с конца массива
          if (_this4.slides[i].tagName !== "BUTTON") {
            //если последний элемент не кнопка, т.е. кнопка пропускается и идет цикл с дивов
            var active = _this4.slides[i];

            _this4.container.insertBefore(active, _this4.slides[0]); //при нажатии на пред. последний элемент будет отправлен в начало //Метод Node.insertBefore() добавляет элемент в список дочерних элементов родителя перед указанным элементом.
    ...
    value: function init() {
      ...
      try {
        ...
        this.bindTriggers();
        this.decorizeSlides(); //реализация остановки слайдера по наведению мыши

        this.activateAnimation();
        [this.container, this.next, this.prev].forEach(function (elem) {
          elem.addEventListener('mouseenter', function () {
            //когда наводим мышь на контейнера слайдов
            clearInterval(_this5.paused); //очищаем setInterval
          });
        });
        [this.container, this.next, this.prev].forEach(function (elem) {
          elem.addEventListener('mouseleave', function () {
            //когда убираем мышь с контейнера слайдов
            _this5.activateAnimation(); //запускаем анимацию
...
var Slider = //сразу экспорт класса
function Slider() {
  ...
  //передаем сразу объект ()   
  this.container = document.querySelector(container);

  try {
    this.slides = this.container.children;
  } catch (e) {} //children - это каждый отдельный слайд //try_catch используем т.к. выходит ошибка на 2-й странице module. пытаемся выполнить если только получен котейнер
